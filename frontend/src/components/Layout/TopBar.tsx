...
import { Sun, Moon, Cpu, User, LogOut, Bell, Shield, Lock } from 'lucide-react';
...
export const TopBar: React.FC<TopBarProps> = ({ onOpenAuth, onOpenAlerts }) => {
  const { devices, selectedDevice, setSelectedDevice, wsConnected, alerts } = useDevice();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className={`flex items-center space-x-2 neu-inset px-3.5 py-2 rounded-xl ${
          selectedDevice?.status === 'online' ? 'border border-emerald-500/30' : 'border border-rose-500/40 bg-rose-950/20'
        }`}>
          <span className={`w-2.5 h-2.5 rounded-full neu-dot ${
            selectedDevice?.status === 'online' ? 'neu-dot-emerald animate-pulse' : 'neu-dot-rose'
          }`} />
          <span className={`text-[11px] font-mono font-bold tracking-wider ${
            selectedDevice?.status === 'online' ? 'text-emerald-400' : 'text-rose-400 font-extrabold'
          }`}>
            {selectedDevice?.status === 'online' ? 'HARDWARE ONLINE' : 'HARDWARE OFFLINE'}
          </span>
        </div>

        {/* Cloud Hub Connection Badge */}
        <div className="hidden md:flex items-center space-x-2 neu-inset px-3 py-2 rounded-xl">
          <span className={`w-2 h-2 rounded-full neu-dot ${wsConnected ? 'neu-dot-cyan animate-pulse' : 'neu-dot-rose'}`} />
          <span className="text-[10px] font-mono text-slate-400 font-bold">
            {wsConnected ? 'CLOUD LIVE' : 'HUB OFFLINE'}
          </span>
        </div>
